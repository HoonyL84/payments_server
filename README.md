...
현재는 프로젝트 목표와 경계를 정리하고, Spring Boot 실행 골격, 결제 상태 모델, 멱등 승인 흐름, PG timeout unknown 수렴 흐름까지 구현한 단계입니다.
...
Redis 같은 외부 상태를 사용하는 시점에는 k6 실행 전에 초기화합니다.
...
현재 존재하는 코드는 Spring Boot 애플리케이션, health endpoint, 공통 error response, trace id filter, 결제/취소 상태 모델, 멱등 승인 API 흐름, PG timeout 이후 confirm 흐름과 결제 상태 조회 API입니다.
